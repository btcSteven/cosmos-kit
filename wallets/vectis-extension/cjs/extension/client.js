...
class VectisClient {
    ...
    async getAccount(chainId) {
        const { address, algo, pubkey, name, isNanoLedger, isVectisAccount } = await this.client.getKey(chainId);
        return {
            username: name,
            address,
            algo: algo,
    ...
    async addChain({ chain, name, assetList, preferredEndpoints }) {
        const chainInfo = (0, keplr_1.chainRegistryChainToKeplr)(chain, assetList ? [assetList] : []);
...
exports.VectisClient = VectisClient;
