...
self.onmessage = async ({ data }) => {
    if (data.type === 'init_1') {
        try {
            ...
            const encryptedPublicKey = await eccrypto.encrypt(clientPublicKey, eccrypto.getPublic(workerPrivateKey));
    ...
    if (data.type === 'request_sign') {
        ...
        try {
            // Verify signature.
            await eccrypto.verify(clientPublicKey, hashObject(data.payload), Buffer.from(data.signature));
            if (data.payload.data.type === 'direct') {
                const response = await (await DirectSecp256k1Wallet.fromKey(walletPrivateKey, data.payload.chainBech32Prefix))
                    // @ts-ignore
